use crate::db::config_dao::{state_private_key, state_public_key};
use crate::models::api::TmsApiErrorResult;
use jsonwebtoken::{
...
impl TryFrom<&String> for OAuthState {
    ...
    fn try_from(state_string: &String) -> Result<Self, TmsApiErrorResult> {
        let key = match DecodingKey::from_rsa_pem(state_public_key().as_bytes()) {
            Ok(key) => key,
            Err(err) => {
                return Err(TmsApiErrorResult {
                    message: err.to_string(),
                });
            }
        };

        let token_data: TokenData<OAuthState> =
            match decode(state_string, &key, &Validation::new(Algorithm::RS256)) {
                Ok(data) => data,
                Err(err) => {
                    return Err(TmsApiErrorResult {
                        message: err.to_string(),
...
impl OAuthState {
    pub fn encode(&self) -> Result<String, TmsApiErrorResult> {
        ...
        let key = match EncodingKey::from_rsa_pem(state_private_key().as_bytes()) {
            Ok(key) => key,
            Err(err) => {
                return Err(TmsApiErrorResult {
                    message: err.to_string(),
